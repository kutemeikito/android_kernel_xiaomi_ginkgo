				    ...
				    enum channel chn, int book, int page)
{
	...
	if ((chn & channel_left) || (pTAS2562->mnChannels == 1)) {
		...
		if (pTAS2562->mnLCurrentBook != book) {
			nResult = regmap_write(pTAS2562->regmap,
			    ...
			    unsigned int reg, unsigned int *pValue)
{
	...
	nResult = tas2562_change_book_page(pTAS2562, chn, TAS2562_BOOK_ID(reg),
					   TAS2562_PAGE_ID(reg));
	if (nResult < 0)
		goto end;
	...
	if (nResult < 0)
		...
		dev_dbg(pTAS2562->dev,
			...
			TAS2562_PAGE_ID(reg), TAS2562_PAGE_REG(reg), *pValue);
				  ...
				  unsigned char *pData, unsigned int nLength)
{
	...
	if ((chn & channel_right) && (pTAS2562->mnChannels == 2)) {
		...
		if (nResult < 0)
			...
			dev_dbg(pTAS2562->dev,
				"%s: %x:BOOK:PAGE:REG %u:%u:%u, len: 0x%02x\n",
				__func__, pTAS2562->client->addr,
				TAS2562_BOOK_ID(reg), TAS2562_PAGE_ID(reg),
				TAS2562_PAGE_REG(reg), nLength);
	}
				 ...
				 unsigned char *pData, unsigned int nLength)
{
	...
	nResult = regmap_bulk_read(pTAS2562->regmap, TAS2562_PAGE_REG(reg),
				   pData, nLength);
	if (nResult < 0)
		dev_err(pTAS2562->dev, "%s, ERROR, L=%d, E=%d\n", __func__,
			__LINE__, nResult);
		...
		dev_dbg(pTAS2562->dev,
			...
			TAS2562_PAGE_ID(reg), TAS2562_PAGE_REG(reg), nLength);
end:
	mutex_unlock(&pTAS2562->dev_lock);
	return nResult;
}
				   ...
				   unsigned int mask, unsigned int value)
{
	...
	if ((chn & channel_left) || (pTAS2562->mnChannels == 1)) {
		pTAS2562->client->addr = pTAS2562->mnLAddr;
		nResult = regmap_update_bits(
			pTAS2562->regmap, TAS2562_PAGE_REG(reg), mask, value);
		if (nResult < 0)
			dev_err(pTAS2562->dev, "%s, ERROR, L=%d, E=%d\n",
				__func__, __LINE__, nResult);
		else
...
static void irq_work_routine(struct work_struct *work)
{
	...
	if (chn && channel_left)
		nResult = pTAS2562->read(pTAS2562, channel_left,
					 TAS2562_LatchedInterruptReg0,
					 &nDevInt1Status);
	if (nResult >= 0)
		nResult = pTAS2562->read(pTAS2562, channel_left,
					 TAS2562_LatchedInterruptReg1,
					 &nDevInt2Status);
	else
		goto reload;
	...
	if (chn == channel_right)
		nResult = pTAS2562->read(pTAS2562, channel_right,
					 TAS2562_LatchedInterruptReg0,
					 &nDevInt3Status);
	if (nResult >= 0)
		nResult = pTAS2562->read(pTAS2562, channel_right,
	...
	if (((nDevInt1Status & 0x7) != 0) || ((nDevInt2Status & 0x0f) != 0) ||
		...
		while (nCounter > 0) {
			...
			nResult = pTAS2562->bulk_write(
				pTAS2562, chn, TAS2562_ICN_REG, pICN, 4);
		...
		if ((nDevInt1Status &
			...
			dev_err(pTAS2562->dev,
				...
				__func__, TAS2562_PowerControl, nDevInt1Status);
...
{
	struct tas2562_priv *pTAS2562 =
		container_of(timer, struct tas2562_priv, mtimer);
...
static int tas2562_parse_dt(struct device *dev, struct tas2562_priv *pTAS2562)
{
	...
	if (rc) {
		...
		dev_dbg(pTAS2562->dev, "ti,channels=%d", pTAS2562->mnChannels);
	}
			     ...
			     const struct i2c_device_id *id)
{
	...
	pTAS2562 = devm_kzalloc(&pClient->dev, sizeof(struct tas2562_priv),
				GFP_KERNEL);
	if (pTAS2562 == NULL) {
	...
	if (gpio_is_valid(pTAS2562->mnResetGPIO2)) {
		nResult =
			gpio_request(pTAS2562->mnResetGPIO2, "TAS2562_RESET2");
		if (nResult) {
			dev_err(pTAS2562->dev,
				"%s: Failed to request gpio %d\n", __func__,
				pTAS2562->mnResetGPIO2);
	...
	/* Reset the chip */
	nResult = tas2562_dev_write(pTAS2562, channel_both,
	...
	if (gpio_is_valid(pTAS2562->mnIRQGPIO)) {
		...
		tas2562_dev_write(pTAS2562, channel_both,
		...
		nResult = request_threaded_irq(
			pTAS2562->mnIRQ, tas2562_irq_handler, NULL,
			IRQF_TRIGGER_FALLING | IRQF_ONESHOT, pClient->name,
			pTAS2562);
		if (nResult < 0) {
			dev_err(pTAS2562->dev, "request_irq failed, %d\n",
				nResult);
			goto err;
		}
		disable_irq_nosync(pTAS2562->mnIRQ);
	...
	if (gpio_is_valid(pTAS2562->mnIRQGPIO2) &&
		...
		INIT_DELAYED_WORK(&pTAS2562->irq_work, irq_work_routine);
		nResult = request_threaded_irq(
			pTAS2562->mnIRQ2, tas2562_irq_handler, NULL,
			IRQF_TRIGGER_FALLING | IRQF_ONESHOT, pClient->name,
			pTAS2562);
		if (nResult < 0) {
			dev_err(pTAS2562->dev, "request_irq failed, %d\n",
	...
	if (nResult < 0) {
		dev_err(pTAS2562->dev, "register codec failed, %d\n", nResult);
		goto err;
	}
	//#endif
...
static const struct i2c_device_id tas2562_i2c_id[] = { { "tas2562", 0 }, {} };
