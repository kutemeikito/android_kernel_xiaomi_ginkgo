...
#ifdef CONFIG_ANT_SYS
static ssize_t ant_state_show(struct class *class, struct class_attribute *attr,
			      char *buf)
{
	int state;
...
static int ant_register_class_dev(struct ant_check_info *ant_info)
{
	...
	if (!ant_info->ant_sys_class) {
		ant_info->ant_sys_class =
			class_create(THIS_MODULE, "ant_class");
		if (IS_ERR(ant_info->ant_sys_class)) {
			ant_info->ant_sys_class = NULL;
			pr_err("could not allocate ant_class\n");
			return -1;
		}
	}
...
static int ant_probe(struct platform_device *pdev)
{
	...
	if (pdev->dev.of_node) {
		...
		if (err) {
			dev_err(&pdev->dev,
				"Macle ant_probe DT parsing failed\n");
			goto free_struct;
		}
	} else {
		return -ENOMEM;
